...
\begin{model}[Mack chain ladder] \label{model:mack} \leavevmode
  \begin{enumerate}[label=\bf{\textup{(}Mack\arabic*\textup{)}},ref=\textup{(}Mack\arabic*\textup{)}, wide]
    \item \label{eq:mack-expectation} There exist development factors $f_1, \dots, f_{I - 1}$ such that
          \begin{equation}
            \mathbb{E}[C_{ij} \ \Vert \ C_{i, j - 1}, \dots, C_{i1}] = \mathbb{E}[C_{ij} \ \Vert \ C_{i, j - 1}] = f_{j - 1} C_{i, j - 1}\,
          \end{equation}
          for $1 \leq i \leq I$.
    \item \label{eq:mack-variance} There exist parameters $\sigma_1, \dots, \sigma_{I - 1}$ such that
          \begin{equation}
...
\begin{table}[!htb]
  \centering
  \begin{tabular}{m{5em} m{5em} m{5em} m{5em} m{5em}}\toprule%
...
\begin{figure}[!htb]
  \begin{subfigure}{0.45 \textwidth}
    \includegraphics{mack_semiparam_cond}
    \subcaption{Conditional}
  \end{subfigure}
  \begin{subfigure}{0.45 \textwidth}
    \includegraphics{mack_semiparam_uncond}
    \subcaption{Unconditional}
  \end{subfigure}
...
\end{equation}
It must be stressed that $Q$ has no probabilistic significance: it does not, in general, correspond to the log-likelihood of any distribution. Rather, it functions a device to obtain estimates of the desired parameters, fulfilling in this a similar role to that of the log-likelihood, which is why we refer to it as a \emph{quasi-likelihood function}\sepfootnote{fn:quasi-likelihood}. This extension of the classical GLM framework allows for more flexibility in our modelling, since we only have to assume a particular mean-variance relation for the response, not a distribution. It is usual to identify quasi-likelihood models derived from specific distributions (i.e.\ using the corresponding variance function) by prefixing 'quasi' to the name of said distribution, e.g.\ quasi-Poisson or quasi-binomial. The derived quasi-model yields the same parameter estimates as the classical GLM if the data follow the original distribution. This is the case for the data in \cref{tab:uk-motor}, as it consists of positive integers, hence the estimates shown in \cref{tab:pois-bench} apply to both models, with the exception of the dispersion parameter (which is always equal to $1$ for \modelref{model:poisson}).
...
We are now finally in a position to describe the overdispersed variant of \modelref{model:poisson}.
...
Finally, we consider the parametric bootstrap. Here again, the development proceeds in a very similar way to \cref{sec:mack-boot}. We begin by fitting the model to the original data, giving us estimates for $c$, $\alpha_i$ and $\beta_j$, as well as $\phi$ if we're using \modelref{model:odp}. With the help of a random number generator, we can then use these to generate a new upper triangle. For \modelref{model:poisson}, this means sampling from a Poisson distribution with mean $\widehat{\mu}_{ij} = \exp{( \widehat{c} + \widehat{\alpha}_i + \widehat{\beta}_j )}$; for \modelref{model:poisson} any distribution satisfying
\begin{equation}
  \var{X_{ij}} = \widehat{\phi} \, V(\widehat{\mu}_{ij}) = \phi \, \widehat{\mu}_{ij}
\end{equation}
will do. The model is then refitted to the simulated triangle to obtain $\widehat{c}^{(k)}$, $\widehat{\alpha}^{(k)}_i$, $\widehat{\beta}^{(k)}_j$ and $\widehat{\phi}^{(k)}$.
...
As explained in \cref{chapter:intro,sec:boot-proc}, we want to investigate the effect of deviations from the model assumptions on the predictive distribution of the total outstanding claims. To this end, we simulate a large number of run-off triangles which conform perfectly to these assumptions, perturb them in a number of ways, and use the bootstrap methods described in \cref{chapter:mack,chapter:poisson} to compute resulting distribution. We broadly follow the methodology outlined in \cite{schiegl}, adapted, of course, to the models under consideration.

\section{Mack's model}

Recall from \cref{sec:mack-intro} that \modelref{model:mack} is recursive in nature, with the cumulative claim amounts in the next development determined by the observations in the previous one. \Modelref{model:mack} therefore requires the first column of the triangle to be given. We can either define a seperate model for it and simulate from that, or simply take the. We also require a set of development factors $f_1, \dots, f_{I - 1}$ and dispersion parameters $\sigma_1, \dots, \sigma_{I - 1}$. Starting from the initial column, we can then use a data-generating process satisfying.
...
\begin{algorithm}[!htb]
  \begin{algorithmic}
    \Require{Development factors $f_1, \dots, f_I$, dispersion parameters $\sigma_1, \dots, \sigma_I$, perturbed point $(i^*, j^*)$, perturbation factors $c_\mu$, $c_\sigma$}
    \ForAll{$1 \leq i, j \leq I$}
      \State $C_{ij} \gets 0$
    \EndFor
    \For{$i \gets 1, I$}
    ...
    \State \Return{$(C^*_{ij})_{1 \leq i,j \leq I}$}
  \end{algorithmic}
  \caption{Simulating triangle with single perturbed point}
\end{algorithm}
...
\begin{algorithm}[!htb]
  \begin{algorithmic}
    ...
    \State \Return{$(C^*_{ij})_{1 \leq i,j \leq I}$}
  \end{algorithmic}
  \caption{Simulating a clean triangle conforming to \cref{model:poisson}}
\end{algorithm}
