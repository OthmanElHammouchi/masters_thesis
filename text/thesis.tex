...
\begin{figure}
  \centering
  \includegraphics{original_resids}
...
Using the taxonomy from \Cref{subsec:boot-reg}, we now consider in turn the application of the semiparameteric, nonparametric and parametric type to \Cref{model:mack}. For the semiparametric bootstrap, the crucial step is to find a suitable definition for the residuals which ensures that they are interchangeable. The distribution-free nature of the model makes this difficult, however, as does not allow us to make precise statements about the distribution of the errors. 
...
\end{equation}
which can easily be seen to be compatible with \Cref{model:mack}. Because the Mack CL can be viewed as a series of weighted linear regressions, as we say in \Cref{subsec:mack-intro}, this has the benefit of making available to us the results of classical regression theory. We know, for example, that the \emph{externally studentised residuals}
\begin{equation}
  r_{i, j + 1} 
...
\begin{table}
  \begin{subtable}{\textwidth}
    ...
    \end{tabular}
    \subcaption{Conditional resampling}
  \end{subtable}
  \par\bigskip
  \begin{subtable}{\textwidth}
    \centering
    ...
    \end{tabular}
    \subcaption{Unconditional resampling}
  \end{subtable}
  ...
  \begin{algorithmic}
    ...
    \For{$b \gets 1, B$}
      ...
      \Else
        \For{$j \gets 1, I - 1$}
          \vspace{5pt}
          \State $\{ (\widetilde{C}^{(b)}_{i, j}, C^{(b)}_{i, j + 1}) \mid i = 1, \dots, I - j \} \gets$ \Call{resample}{$\{ (C^{(b)}_{i, j}, C_{i, j + 1}) \mid i = 1, \dots, I - j \}$}
          \vspace{5pt}
          \State $\widehat{f}^{(b)}_j \gets \sum_{i = 1}^{I - j} C^{(b)}_{i, j + 1} / \sum_{i = 1}^{I - j} C^{(b)}_{ij}$
          \vspace{5pt}
          \For{$i \gets 1, I - j$}
