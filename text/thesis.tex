...
\end{equation}
While this does not directly address our present concern, namely of bootstrapping the parameters in \Cref{model:mack}, it suggests that our woes may be remedied through replacement of the Gaussian distribution in \Cref{eq:time-series-model} with a different one capable of guaranteeing
...
semiparametric bootstrap are outlined in \Cref{alg:semiparam-pois}
...
Next, we move on to the parametric bootstrap. Here again, the development proceeds in a very similar way to \Cref{sec:mack-boot}. We begin by fitting the model to the original data, giving us estimates for $\alpha_i$
...
Finally, we consider the question of incorporating the intrinsic variability of the response in our bootstrap simulations. Just as in \Cref{sec:boot-proc} and \Cref{sec:mack-proc}, we will use the concept of the predictive distribution to achieve this. Depending on the type of bootstrap under consideration, this will result in different procedures. For the semiparametric bootstrap, we resample the residuals a second time and invert the appropriate formula to obtain bootstrap responses.
...
Recall from \Cref{sec:mack-intro} that \Cref{model:mack} is recursive in nature, with the cumulative claim amounts in the next development determined by the observations in the previous one. The Mack CL therefore requires the first column of the triangle to be given. Starting from this,

\begin{algorithm}[!htb]
  \begin{algorithmic}
    \Require{Development factors $f_1, \dots, f_I$, dispersion parameters $\sigma_1, \dots, \sigma_I$}
  ...
  \end{algorithmic}
  \caption{Simulating a clean triangle conforming to \Cref{model:mack}}
\end{algorithm}

\begin{algorithm}[!htb]
  \begin{algorithmic}
    \Require{Development factors $f_1, \dots, f_I$, dispersion parameters $\sigma_1, \dots, \sigma_I$, perturbed point $(i^*, j^*)$, perturbation factor $c_p$}
    \ForAll{$1 \leq i, j \leq I$}
      \State $C_{ij} \gets 0$
    \EndFor
    \For{$i \gets 1, I$}
    ...
    \Else
      ...
      \EndFor
      \State $C^*_{i^*j^*} \gets \mathcal{N}(c_p f_j C^*_{i^*, j^* - 1}, \sigma_j^2 C^*_{i^*, j^* - 1})$
    \EndIf
    \EndFor
    \State \Return{$(C^*_{ij})_{1 \leq i,j \leq I}$}
  \end{algorithmic}
  \caption{Simulating triangle with single mean-perturbed point}
\end{algorithm}
  ...
  \begin{algorithmic}
    ...
    \Else
      \For{$j \gets 1, j^* - 1$}
        \State $C^*_{i^*j} \gets \mathcal{N}(f_j C^*_{i^*, j - 1}, \sigma_j^2 C^*_{i^*, j - 1})$
      \EndFor
      \State $C^*_{i^*j^*} \gets \mathcal{N}(f_j C^*_{i^*, j^* - 1}, c_p \, \sigma_j^2 C^*_{i^*, j^* - 1})$
    \EndIf
    \EndFor
    \State \Return{$(C^*_{ij})_{1 \leq i,j \leq I}$}
  \end{algorithmic}
  \caption{Simulating triangle with single variance-perturbed point}
\end{algorithm}
