...
As explained in the \nameref{intro} and \cref{sec:boot-proc}, we want to study the impact of deviations from the model assumptions on the predictive distribution of the total outstanding claims. To this end, we simulate a large number of run-off triangles, following a similar approach to the one described in \cite{schiegl}, which satisfy these assumptions perfectly except for a number of deviating observations. We then use the bootstrap methods described in \cref{chapter:mack,chapter:poisson} to compute the corresponding distribution while excluding a subset of points from the resampling process. Our aim is to investigate whether the predictive distributions computed from data where the correct points have been discarded differ significantly from those obtained after removing arbitrary observations.
...
In general, there seems to be little difference between conditional and unconditional.

In experiments where a single point has been perturbed, we observe no difference the semiparametric bootstraps with standardised and log-normal residuals perform

we observe no significant difference between the predictive distribution with and without
...
\begin{table}
  \begin{subtable}{0.45\linewidth}
    ...
    \label{fig:resids-standard-example}
  \end{subtable}
  \hfill
  \begin{subtable}{0.45\linewidth}
    \centering
    ...
    \label{fig:resids-log-normal-example}
  \end{subtable}
  \caption{Residuals corresponding to \cref{fig:sim-triangle-example}}
\end{table}

\label{fig:resids-studentised-example}
...
\begin{landscape}
  \begin{figure}
    ...
    \caption{Density plots of predictive distributions obtained using the unconditional semiparametric bootstrap with log-normal residuals on Mack's model for different deviating points. The curves in each facet correspond to densities obtained while excluding a different point in the resampling procedure, with the result from the cleaned dataset highlighted in red. The perturbation factors are $c_\mu = c_\sigma = 2$.}
    \label{fig:mack-single-uncond-semiparam-lognormal}
  \end{figure}
\end{landscape}
...
\begin{landscape}
  \begin{figure}
    ...
    \caption{Density plots of predictive distributions obtained using the conditional parametric bootstrap with the normal distribution on Mack's model for different deviating points. The curves in each facet correspond to densities obtained while excluding a different point in the resampling procedure, with the result from the cleaned dataset highlighted in red. The perturbation factors are $c_\mu = 2$ and $c_\sigma = 1$.}
    \label{fig:mack-single-cond-param-normal}
  \end{figure}
\end{landscape}
...
\begin{landscape}
  \begin{figure}
    ...
    \caption{Density plots of predictive distributions obtained using the unconditional parametric bootstrap with the gamma distribution on Mack's model for different deviating points. The curves in each facet correspond to densities obtained while excluding a different point in the resampling procedure, with the result from the cleaned dataset highlighted in red. The perturbation factors are $c_\mu = 0.5$ and $c_\sigma = 1$.}
...
\begin{landscape}
  \begin{figure}
    ...
    \end{subfigure}
    \caption{Density plots of predictive distributions obtained using the semiparametric bootstrap with standardised residuals on Mack's model for different deviating calendar periods}
  \end{figure}
\end{landscape}
...
\begin{landscape}
  \begin{figure}
    ...
    \end{subfigure}
    \caption{Density plots of predictive distributions obtained using the semiparametric bootstrap with log-normal residuals on Mack's model for different deviating calendar periods}
  \end{figure}
\end{landscape}
...
\begin{landscape}
  \begin{figure}
    ...
    \end{subfigure}
    \caption{Density plots of predictive distributions obtained using the parametric bootstrap on Mack's model for different deviating calendar periods}
  \end{figure}
\end{landscape}
...
\input{../results/mean_table_1.tex}
\input{../results/mean_table_2.tex}
