...
results <- foreach(config = iter(config, by = "row"), .combine = "rbind", .errorhandling = "stop") %dopar% {
                ...
                reserve <- reserveBoot(distortedTriangle, 1e3,
                    distribution = "normal",
...
if (exists("results/data_objects/single_outlier.RDS")) {
    ...
    indices <- expand.grid(seq_len(nrow(points)), seq_len(length(pertfac)), seq_len(nrow(points)), seq_len(length(residual.types)), seq_len(length(bootstrap.types)), seq_len(length(distributions)))

    config <- cbind.data.frame(points[indices[, 1], ], pertfac[indices[, 2]], points[indices[, 3], ], residual.types[indices[, 4]], bootstrap.types[indices[, 5]], distributions[indices[, 6]])
    ...
    results <- foreach(config = iter(config, by = "row"), .combine = "rbind", .errorhandling = "stop") %dopar% {
        ...
        reserve <- reserveBoot(distortedTriangle, 1e3, exclude.residuals = exclude.residuals, distribution = distribution, resids.type = resids.type, bootstrap.type = bootstrap.type)
