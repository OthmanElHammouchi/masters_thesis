...
```{r}
results <- bootReserve(claimsTriangle, 1e3)

claimsTriangle <- singleOutlier(nDev, initMean, initStd, devFacs, sigma, 4, 3, pert = 0.5)

sigmaProblem <- list()
sigmaOK <- list()
for (i in 1:1e3) {
    if (NA %in% results$triangleBoot[[i]] || (NaN %in% results$triangleBoot[[i]])) {
        sigmaProblem %<>% c(., list(results$sigmaBoot[[i]]))
    } else {
        sigmaOK %<>% c(., list(results$sigmaBoot[[i]]))
    }
}

sigmaProblem %<>% unlist() %>% tibble(sigmaProblem = .)
sigmaOK %<>% unlist() %>% tibble(sigmaOK = .)

p1 <- ggplot(data = sigmaProblem) +
    geom_histogram(aes(sigmaProblem))

p2 <- ggplot(data = sigmaOK) +
    geom_histogram(aes(sigmaOK))
...
for (colIdx in 2:nDev) {
    for (rowIdx in 1:(nDev + 1 - colIdx)) {
        for (pert in seq(0.5, 1.5, length.out = 10)) {
            triangle <- singleOutlier(nDev, initMean, initStd, devFacs, sigma, colIdx, rowIdx, pert = pert)
...
claimsTriangle <- singleOutlier(nDev, initMean, initStd, devFacs, sigma, colIdx, rowIdx, pert = 0.9)
